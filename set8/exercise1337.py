...
def string_list_please() -> list:
    ...
    str_list = ["cats", "dogs", "mice"]
    return str_list


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    my_dict = {"colour": "red", "size": "big", "shape": "circle"}
    return my_dict
...
def best_letter_for_pets() -> str:
    ...
    for i in the_alphabet:
        count_list.append(len(pet_filter(i)))
    most_popular_letter = str(the_alphabet[count_list.index(max(count_list))])
...
def random_filler_text(number_of_words=200) -> str:
    ...
    for i in range(number_of_words):
        word = str(random.choice(random.choice(list(my_dict.values()))))
        words.append(word)
...
def fast_filler(number_of_words=200) -> str:
    ...
    words = []
    for i in range(number_of_words):
        word = str(random.choice(random.choice(list(contents.values())))).title()
        words.append(word)
